...
async function upCommand(args: string[], root: string): Promise<void> {
  ...
  console.log(`  Starting ${ui.bold("review")} (Review Worker, inspector 9231)...`);
  spawnService({
    name: "review",
    command: "pnpm exec wrangler dev --inspector-port 9231",
    cwd: join(root, "cloudflare-code-review-infra"),
  });
...
async function testWebhookCommand(args: string[], _root: string): Promise<void> {
  const WEBHOOK_URL =
    process.env.WEBHOOK_URL ?? "http://127.0.0.1:3000/api/webhooks/github";
...
export const codeReview: ProjectDef = {
  name: "code-review",
  description: "Code review dev environment (workers + Next.js)",
  commands: {
    up: {
      description:
        "Start code review dev environment (Next.js + session/review/agent-next workers)",
      run: upCommand,
    },
    "test-webhook": {
      description:
        "Send a test GitHub pull_request webhook to the local dev server",
