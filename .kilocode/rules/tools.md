...
- DO NOT RUN `pnpm run dev`! The webserver should already be running. Read the dev server port from the `.dev-port` file in the project root (e.g., `cat .dev-port`), then access it via `http://localhost:<port>/`. If `.dev-port` does not exist, fall back to port 3000. If the server does not appear to be running, EVEN THEN ASK FOR PERMISSION BEFORE RUNNING IT.
