...
## Bonus: `forceSandbox` Already Exists
There's already a `forceSandbox` mechanism in the codebase that can be leveraged for the "Deep" mode.
...
- **`deep`**: Skip triage (or ignore its recommendation). Always run sandbox analysis (leverage existing `forceSandbox` mechanism).
