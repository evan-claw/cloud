...
/**
 * Retroactively expire a specific credit transaction for a user.
 * Sets expiry_date to `now` on the target transaction, then runs
 * processLocalExpirations to settle the balance.
 *
 * The target transaction must be free, non-expiring, non-organization,
 * and not already processed.
 *
 * Returns the user's balance (total_microdollars_acquired - microdollars_used)
 * before and after, or null if the transaction was not eligible.
 */
export async function retroactivelyExpireCreditsForUser(
  ...
  // Verify the target transaction is eligible (free, non-expiring, not already processed)
  ...
  await db
    ...
    .where(eq(creditTransactionsTable.id, creditTransactionId));
